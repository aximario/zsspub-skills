...
基于本地 SQLite 数据库记录和管理每日打卡数据。数据文件由脚本通过 `import.meta.dirname` 自动定位，存放在与 skill 目录同级的 `.data/zsspub-checkin/` 目录下，无需手动配置路径。时间以 UTC 存入数据库，展示时自动按用户配置的时区进行转换。
...
```bash
node <本 SKILL.md 所在目录>/scripts/checkin.mjs <命令> [选项]
```

下文中 `checkin` 均代表 `node <skill目录>/scripts/checkin.mjs`，请替换为实际路径。
...
```bash
checkin config --timezone=Asia/Shanghai
```
...
### 添加打卡
```
checkin add "主题" [--tags=标签1,标签2] [--duration=分钟数] [--note="备注"] [--raw="用户原话"] [--at="YYYY-MM-DD HH:mm:ss"]
```
...
### 查看打卡记录
```
checkin list [--topic=主题] [--tag=标签] [--date=YYYY-MM-DD] [--from="YYYY-MM-DD HH:mm:ss"] [--to="YYYY-MM-DD HH:mm:ss"] [--search=关键字] [--limit=数量]
```
- `--date`：查看某天的所有打卡
- `--from` / `--to`：时间范围筛选
- `--search`：按主题和备注模糊搜索
...
### 修改打卡
```
checkin update <id> [--topic=...] [--tags=...] [--duration=...] [--note=...] [--at="YYYY-MM-DD HH:mm:ss"]
```
- 清除时长：`--duration=null`
...
### 统计分析
```
checkin stats [--topic=主题] [--tag=标签] [--period=today|week|month|year] [--from="YYYY-MM-DD HH:mm:ss"] [--to="YYYY-MM-DD HH:mm:ss"]
```
...
### 连续打卡
```
checkin streak [--topic=主题] [--tag=标签]
```
- 输出：当前连续天数、最长连续天数、累计打卡天数

### 查看/更新配置
```
checkin config [--timezone=<IANA时区>]
```
...
```bash
# 打卡骑行 30 分钟
checkin add "骑行" --tags=运动 --duration=30 --raw="打卡骑行 30 分钟"

# 打卡阅读，带备注
checkin add "阅读" --tags=学习 --note="《原则》第三章"

# 补打卡（昨天的跑步）
checkin add "跑步" --tags=运动 --duration=40 --note="5公里" --at="2026-04-01 19:30:00"

# 查看今天的打卡
checkin list --date=2026-04-02

# 查看所有骑行记录
checkin list --topic=骑行

# 本周运动统计
checkin stats --period=week --tag=运动

# 查看骑行连续打卡天数
checkin streak --topic=骑行

# 修改第 2 条打卡的时长
checkin update 2 --duration=45
