---
name: todos
description: "个人待办事项管理。当用户需要记一些简短的事情的时候使用。"
metadata:
  version: "3.0.0"
...
### POST /todos — 创建
```bash
curl -s -X POST https://zss.pub/todos -H "x-access-key: $ACCESS_KEY" -H "Content-Type: application/json" \
...
### GET /todos — 列出
```bash
curl -s "https://zss.pub/todos?status=pending&priority=high&tag=工作&search=关键字" -H "x-access-key: $ACCESS_KEY"
```
参数：
...
```bash
curl -s -X PATCH https://zss.pub/todos/3/done -H "x-access-key: $ACCESS_KEY"
```

### PATCH /todos/:id — 更新
```bash
curl -s -X PATCH https://zss.pub/todos/2 -H "x-access-key: $ACCESS_KEY" -H "Content-Type: application/json" \
  -d '{"title":"新标题","priority":"high","dueDate":null}'
```

### DELETE /todos/:id — 删除
```bash
curl -s -X DELETE https://zss.pub/todos/5 -H "x-access-key: $ACCESS_KEY"
```
响应：204 No Content
